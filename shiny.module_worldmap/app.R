...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      # select continents
      selectInput(inputId = "filter_country", 
                  label = "Filter by Country (multiple):",
                  choices = 
                    list("North America" = c("United States of America", 
                                              "Canada", "Belize",
                                             "Costa Rica", "Cuba" , 
                                             ...
                                             "Trinidad and Tobago" ), 
                         "South America" = c("Argentina", "Bolivia", "Brazil", 
                                             "Chile", "Colombia", "Ecuador", 
                                             "Falkland Islands", "Guyana", 
                                             "Paraguay", "Peru", "Suriname", 
                                             "Uruguay", "Venezuela"), 
                         "Africa" = c("Algeria", "Angola", "Benin", "Botswana", 
...
server <- function(input, output) {
  ...
  output$world_map <- renderPlot({
    ggplot() + 
      geom_sf(data = countries110[countries110$geounit %in% input$filter_country,], 
